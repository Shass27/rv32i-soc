...
// slave responses (ack, dat, err) are driven manually to isolate master FSM behaviour
...
module tb_wb_master;
    ...
    reg         i_wb_err;  // fake slave err

    wb_master dut (
        .i_clk(clk), .i_rst(rst),
        .i_req(i_req), .i_we(i_we), .i_addr(i_addr), .i_wdat(i_wdat),
        .o_rdat(o_rdat), .o_busy(o_busy), .o_done(o_done), .o_err(o_err),
    ...
    initial begin
        ...
        // -- reset --
        i_req = 0; i_we = 0; i_addr = 0; i_wdat = 0; i_wb_dat = 0; i_wb_ack = 0; i_wb_err = 0;
        ...
        @(negedge clk);
        i_req = 1; i_we = 1; i_addr = `WB_RAM_BASE + 32'h10; i_wdat = 32'hDEAD_BEEF;
        ...
        check(o_wb_adr, `WB_RAM_BASE + 32'h10, "T2_ADR");
        ...
        @(negedge clk);
        i_req = 1; i_we = 0; i_addr = `WB_RAM_BASE + 32'h20; i_wdat = 0; i_wb_dat = 32'hCAFE_BABE;
        @(posedge clk); #1; i_req = 0;
        check(o_wb_we, 0, "T3_WE=0_read");
        check(o_wb_adr, `WB_RAM_BASE + 32'h20, "T3_ADR");
        i_wb_ack = 1; @(posedge clk); #1; i_wb_ack = 0;
        ...
        @(negedge clk);
        i_req = 1; i_we = 0; i_addr = `WB_RAM_BASE + 32'h30; i_wdat = 0; i_wb_dat = 32'h5A5A;
        @(posedge clk); #1; i_req = 0;
        ...
        @(negedge clk);
        i_req = 1; i_we = 1; i_addr = `WB_RAM_BASE + 32'h40; i_wdat = 32'hFF;
        ...
        check(o_wb_adr, `WB_RAM_BASE + 32'h40, "T5_addr_unchanged_during_busy"); // still the first txn
        i_wb_ack = 1; @(posedge clk); #1; i_wb_ack = 0;
        check(o_busy, 0, "T5_busy_cleared");

        // T6: back-to-back transactions
        @(negedge clk);
        i_req = 1; i_we = 1; i_addr = `WB_RAM_BASE + 32'h50; i_wdat = 32'hAA;
        @(posedge clk); #1; i_req = 0;
        i_wb_ack = 1; @(posedge clk); #1; i_wb_ack = 0; // finish first
        check(o_busy, 0, "T6_idle_between_txns");
        @(negedge clk);
        i_req = 1; i_we = 0; i_addr = `WB_RAM_BASE + 32'h54; i_wb_dat = 32'hBB;
        @(posedge clk); #1; i_req = 0;
        i_wb_ack = 1; @(posedge clk); #1; i_wb_ack = 0;
        check(o_rdat, 32'hBB, "T6_second_txn_rdat");

        // T7: ERR terminates transaction — done pulses, err qualifies it
        @(negedge clk);
        i_req = 1; i_we = 0; i_addr = `WB_ERR_BASE; i_wdat = 0;
        @(posedge clk); #1; i_req = 0;             // master ACTIVE
        check(o_wb_cyc, 1, "T7_CYC=1_before_err");
        i_wb_err = 1; @(posedge clk); #1; i_wb_err = 0;
        check(o_done, 1, "T7_done=1_on_err");
        check(o_err,  1, "T7_err=1_on_err");
        check(o_wb_cyc, 0, "T7_CYC=0_after_err");
        check(o_busy,   0, "T7_busy=0_after_err");
        @(posedge clk); #1;                         // next cycle — both flags gone
        check(o_done, 0, "T7_done=0_next_cycle");
        check(o_err,  0, "T7_err=0_next_cycle");

        // T8: ACK terminates cleanly — o_err stays 0
        @(negedge clk);
        i_req = 1; i_we = 0; i_addr = `WB_RAM_BASE + 32'h60; i_wb_dat = 32'h1234;
        @(posedge clk); #1; i_req = 0;
        i_wb_ack = 1; @(posedge clk); #1; i_wb_ack = 0;
        check(o_done, 1, "T8_done=1_on_ack");
        check(o_err,  0, "T8_err=0_on_ack");
